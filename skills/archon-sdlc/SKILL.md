...
- Start it with the dedicated home: `CODEX_HOME=$HOME/.archon/codex-home` prefixed to the same run line. Preflight fails typed (`BILLING_GUARD=FAIL`) if that home is not ChatGPT-logged-in.
- `maxBudgetUsd` caps are inert and AI-node timeouts are non-lethal under codex — the only brakes are loop caps, the ChatGPT quota (quota wall = typed node failure after 3 retries, resumable), and you. Watch codex runs at least as closely as claude ones.
