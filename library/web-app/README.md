# Skills library: web-app

Repository-specific knowledge compiled from Archon runs on `web-app`. Three
...
- Commits are authored as `archon skill-evolve <skill-evolve@archon.local>`
  and touch only `library/web-app`; `git log -- library/web-app` is the audit trail.
