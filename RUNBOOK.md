...
| `REVIEW_RERAISE round=N reraised=<n>/<total>` *(retired on `full-sdlc-api` together with the `yield-stop.txt` branch; closure convergence, §3c, decides that lane now)* | This round and the previous one applied nothing, and at least half of what this round handled was a finding the waiver ledger had already recorded. The reviewer is re-litigating settled decisions rather than finding new ones. It qualifies the same opt-in `yield-stop.txt` convergence branch as `verdict=DIMINISHING`; it is not a convergence path of its own. | None. Read `waivers.md` if you disagree that those decisions were settled. |
