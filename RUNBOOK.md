...
| Terminal `status=failed` with a `discriminator` field | `supervise_exact_run` read the run's `gate-status.txt`. Every typed line `rca-shape.sh` emits is persisted there, because a failing node's stdout does not survive the executor's failure path -- it reports a fragment of the node's own script source instead. | Route on the discriminator, not on the raw executor message. `cat $ARTIFACTS_DIR/gate-status.txt` for the full typed history of the run's last shape check. |
