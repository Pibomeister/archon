...
The repository skills library lives at `.archon/library/<repo>/` (repo one of
...
tail -40 "$ROOT/.archon/library/<repo>/wiki/skill-impact.md"
git -C "$ROOT/.archon" log --oneline -5 -- library/<repo>
```
...
**Operator levers** (all from `$ROOT`; `LIB="$ROOT/.archon/library"`):

```bash
python3 "$ROOT/.archon/setup/skill-admit.py" status <repo> --lib "$LIB"                     # SKILL_INDEX=OK candidate=<name|none> active=n rolled_back=n
python3 "$ROOT/.archon/setup/skill-score.py" window <repo> --lib "$LIB"                     # SKILL_WINDOW=NONE | OPEN skill=.. runs=n/K baseline=..
python3 "$ROOT/.archon/setup/skill-admit.py" rollback <repo> --lib "$LIB" --reason "why"    # forced rollback of the pending candidate (SKILL_ROLLBACK=OK|SKIP)
