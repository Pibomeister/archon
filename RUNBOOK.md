...
| `SMOKE=FAIL … class=infrastructure reason=stack-down` / `JOINT_E2E=FAIL class=infrastructure …` | Boot or joint e2e blamed a missing docker stack, a stale `.env`, or a missing second-identity subscription — not the feature. A dead process with the stack up has no `class=` and **is** the feature. | Follow the printed `RECOVERY=`. Do not treat `code=000` as a product bug. Recipe: [operator recovery](docs/operator-recovery.md). |
...
- **`SMOKE=FAIL api-docs-json code=000` without `class=infrastructure` is the old shrug.** Since VERSION 2026.09.18-1 a missing `postgres-db` or `dynamodb-local` prints `reason=stack-down class=infrastructure`. A dead api process with the stack up prints `api-boot exited before ready` with no `class=` — that is the feature. Do not stop those containers from a node-scoped cleanup.
