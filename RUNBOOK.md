...
| Verbatim signal | Meaning | Action |
|---|---|---|
| `DESLOP_GATE=FAIL <guard> round=N` — `<guard>` one of `typecheck`, `lint`, `unit`, `scope`, `slop` (both lanes), or `repro no longer green rc=<RP>` (bugfix only; see the harness-error variant below for its own `rc=97`) | `deslop-recheck`'s own mechanical re-verification failed — the writer's cleanup broke something, or (bugfix) moved the repro off green. | Hand-fix the worktree (the failure prints the relevant log tail), then resume — resume re-enters the `loop_group` with a fresh iteration, so `deslop-recheck` re-runs and re-takes the checkpoint. |
...
| `RCA_CONVERGE=FAIL no rca-round.txt` / `RCA_CONVERGE=FAIL rca-round.txt is not an integer: [$N]` / `RCA_CONVERGE=FAIL unknown verdict [$V] round=$N` / `RCA_CONVERGE=FAIL no revision.json round=$N` / `RCA_CONVERGE=FAIL critique.json findings unreadable round=$N` / `RCA_CONVERGE=FAIL shape round=$N` / `RCA_CONVERGE=FAIL revision.json malformed round=$N` | Same seven fail-closed checks as `plan-converge`'s `PLAN_CONVERGE=FAIL` group in §3a, same order, same meanings: (1)-(2) the round counter missing/corrupted (should be unreachable); (3) `critique.json`'s verdict outside the enum; (4) `rca-revise` wrote no `revision.json`; (5) `critique.json`'s `findings` array unparseable for the blocking-count re-derivation; (6) ACCEPT verdict but the post-loop `rca-shape.sh` re-check failed; (7) `revision.json` fails its shape check. | Same as §3a: (1)-(2) hand-inspect `rca-round.txt`, engineer if recurring; (3),(4),(5),(7) resume re-runs the loop fresh, recurring → prompt drift → engineer; (6) resume re-runs the loop, recurring → a reviser is corrupting the contracts → engineer. |
