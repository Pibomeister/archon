...
| `FEATURE_CHAIN=FAIL <repo> reopen produced no change: candidate tree equals previous head <sha>` (advance) | The controller-side belt for the row above: every exit of a reopened stage (gate-tests, commit-impl, a deslop fixer that reverted the fix) is judged against private chain state when the stage result is recorded. | Same as the row above. |
