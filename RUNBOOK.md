...
The per-repo review loop ends in exactly one of five ways. The discriminator strings appear verbatim in the converge output (`round-N/converge.txt` in the run's artifacts dir; stdout is teed there because archon persists bash stderr but not stdout).

**Two shapes of this loop ship.** `bugfix` and `full-sdlc-web` still run v1's `round-pre → review → review-gate → commit-fixes → fixer → commit-fixer → converge`. `full-sdlc-api` and its lite and codex twins run v2's `round-pre → review → review-gate → fix-plan → fixer → commit-fixer → converge`, where `review` and `fixer` are conditional and `commit-fixes` is gone. The five exits below are the same on both. Everything specific to v2 — durable round checkpoints, closure convergence, the pin guard — is in §3c, and a row in the two-hour table below marked *(v1 lanes only)* no longer fires on `full-sdlc-api`.
...
Two-hour convergence work (repository-scope runs) added the typed lines below. Each appears verbatim in `round-N/converge.txt`, the node's own output, or `node-<id>.out`.

| Verbatim signal | Meaning | Action |
|---|---|---|
| `REVIEW_MODE=full base=<sha>` / `REVIEW_MODE=delta base=<sha>` | `round-pre` decided this round's review scope and wrote it to `round-N/review-mode.txt` + `round-N/review-base.txt`. `delta` reviews only what landed since `<sha>`: the previous round's pre-fix HEAD after a P2/P3-only round, otherwise the run's `bootstrap-head.txt` (a stacked chain's parent head, so parents are never re-reviewed). `full` against `origin/main` appears only when `bootstrap-head.txt` is missing or empty. Informational — the node still types its exit with `ROUND=N head=<sha>`. | None. To force a full round, delete `round-N/review-mode.txt` before resuming. |
| `REVIEW_SCOPE=FAIL mode=<m> envelope=[<Scope line>]` (review-gate exits 1) *(`bugfix` and `bugfix-lite` only; `full-sdlc-api` retired the Scope-string compare for the identity gate `GATE_5_input_matches`, §3c)* | The reviewer reviewed a different scope than the round asked for — a delta round that re-read everything, or a full round that read only a slice. The round is billed and its verdict describes the wrong diff, so the gate refuses it. | Resume: the next round re-reviews with a fresh reviewer session. Recurring on the same round means the review prompt drifted — engineer. |
| `REVIEW_RERAISE round=N reraised=<n>/<total>` *(informational only on `full-sdlc-api`: the `yield-stop.txt` branch it used to qualify is RETIRED there, superseded by closure convergence, §3c. The file is still read by `chain-acceptance.py` clause 1, and the lite lane keeps its own single-round branch.)* | This round and the previous one applied nothing, and at least half of what this round handled was a finding the waiver ledger had already recorded. The reviewer is re-litigating settled decisions rather than finding new ones. It qualifies the same opt-in `yield-stop.txt` convergence branch as `verdict=DIMINISHING`; it is not a convergence path of its own. | None. Read `waivers.md` if you disagree that those decisions were settled. |
| `DESLOP_COMPLEXITY_DOWNGRADED fn=<name> file=<f>:<line> confidence=<c>->50` | `deslop-review-gate` kept a reviewer complexity finding but dropped it to non-blocking confidence, because `check-slop.py` filed no `SLOP=FAIL` for that file. `check-slop` is the authority on complexity: it already separates a function this change worsened past the threshold from one that was over it beforehand, and only the first is a blocking finding. | None. The finding still stands in `deslop-review.json` and still appears in the parse line's `downgraded=` count; fix it if you want to, but the gate no longer demands it. |
| `DESLOP_CONTRACT_DOWNGRADE symbol=<s> file=<f>:<line> confidence=<c>->50` / `SLOP=REPORT yagni ... reason=approved-contract` | A repository-chain stage gets `contract-symbols.json` at dispatch, derived from the approval-bound joint plan: for each contract this repo produces, the symbols its description and the approved `plan.md` lines naming the artifact declare, plus `pinned_decisions` entries whose file is in the stage allowlist. A yagni finding on such a symbol (the symbol the finding's line declares, or the exported declaration enclosing a decorator/brace line) is a planned contract with no caller yet, not dead code: `check-slop.py --contract-symbols` reports it instead of failing, and `deslop-review-gate` keeps the reviewer finding at confidence 50. Observed: chain 42b42a13 capped at `DESLOP_ROUND_CAP` on a reserved optional DTO field the fixer correctly refused to delete. | None. If the symbol truly is dead, the plan is what reserved it: replan, do not hand-delete an approved contract field. `DESLOP_GATE=FAIL contract-symbols.json altered` / `DESLOP_REVIEW=FAIL contract-symbols.json altered`: the file no longer hashes to `params.json` `feature_contract_symbols_sha256` (a session widened it); engineer, never re-bind it by hand. |
| `REOPEN=NOOP` | A feature-chain reopen matched no stage to send back — there was nothing to re-implement or re-verify. | Engineer. Read the chain state; a reopen that matched nothing usually means the wrong chain id, or a stage that already advanced past the reopen point. |
| `IMPLEMENT=FAIL reopen produced no change` (gate-tests) | A non-verify-only `feature-reopen` dispatches the stage with `reopen-context.json` (reason, previous candidate head, stopped run, paths to its failing evidence), bound by `params.json` `feature_reopen_context_sha256`. The implement node is told that file is mandatory work; a worktree still identical to the previous candidate did not address it, and passing it would re-verify the exact candidate integration already failed. `REOPEN_GATE=PASS` / `REOPEN_GATE=SKIP no reopen context` are the non-stop forms. | Read the implement transcript and `reopen-context.json`. When the reason is actionable, reopen again with a sharper reason: `archon-run.py feature-reopen --chain <id> --repo <repo> --reason "..."` is admitted for a reopened stage whose own implement run stopped (failed or cancelled, bound after the latest reopen touching that repo), and keeps the original previous head and failure evidence. When the fix already landed by hand, use `--verify-only`. |
| `FEATURE_CHAIN=FAIL <repo> reopen produced no change: candidate tree equals previous head <sha>` (advance) | The controller-side belt for the row above and the verify-only row below (judged against `verify_only_head`): every exit of a reopened stage (gate-tests, commit-impl, a deslop fixer that reverted the fix) is judged against private chain state when the stage result is recorded. | Same as the row above. |
| `IMPLEMENT=FAIL verify-only reopen has no change since previous head <sha>` (gate-tests) / `VERIFY_ONLY=BASE previous=<sha> head=<sha>` / `VERIFY_ONLY=FAIL <reason>` (bootstrap) | `feature-reopen --verify-only` re-verifies a fix already in the stage worktree (committed by a stopped run or by hand). Only the implement node is skipped: `gate-tests` also depends on `implementation-ready` (`none_failed_min_one_success`), so deslop, reader-audit, commit-impl (a no-op when nothing is uncommitted), review-loop, exit-gate, smoke and local-candidate all run and `feature-result.json` is written. Bootstrap's `verify-only-base.py` rewrites `bootstrap-head.txt` to `params.json` `feature_previous_head` (the previous verified candidate), so every gate, the review diff and the candidate squash see previous-candidate..worktree HEAD, and writes `commit-msg.txt` from the oldest commit subject in that range. Before this (run e21573ca) every tail node was skipped, the run completed with no `feature-result.json`, and `feature-advance` failed `stage missing feature-result.json`. | No change: the fix never landed in the worktree; commit it there, then reopen `--verify-only` again. `VERIFY_ONLY=FAIL ... not an ancestor`: the worktree was reset past the previous candidate; engineer. A verify-only run recorded `completed` without `feature-result.json` counts as stopped, so `feature-reopen ... --verify-only` is admitted for it. Resuming it re-runs the captured (old) source; dispatch a fresh reopen instead. The web lane shares the base rewrite and the no-change gate but still runs its implement node, which is told to stop when the work already exists. |
| `IMPLEMENT=FAIL reopen needs files outside the approved allowlist: <files>` | The implementer found the reopen fix needs a file outside `files-allowlist.json` and wrote `reopen-blocked.txt` instead of editing it. Scope is never widened silently. | Human decision. Either `feature-scope-amend <run> --chain <id> --add-file <file> --reason "..."` per file, then reopen again; or reject the reason and replan. |
...
## 3c. Durable round checkpoints and closure convergence (`full-sdlc-api` only)

v1 measured 187 minutes of review across two stages, and five of its thirteen review invocations were **replays**: `resume.sh` hands a failed `loop_group` back to the CLI, which restarts it at `round-pre`, and every node behind it re-ran whether or not its work was already on disk. v2 gives each activity a durable completion record, so a resume re-runs only the activity that was interrupted.

**The state lives in `round-N/`, not in node outputs.** `setup/round-state.py` is the only writer; every file is written `tmp` + `mv`, so a kill leaves either the old file or the new one and never half of one. The files that decide things, in the order they appear:

| File | Written by | Means |
|---|---|---|
| `review-input.json` | `round-pre`, when it decides `review: run` | the review's inputs: candidate head, the tree sha before the reviewer started, base, scope, and the contract/plan/allowlist digests |
| `review-envelope.txt` | the reviewer session, last action | the review itself; a reviewer that does not call `mark review-done` leaves an *interrupted* review, not a finished one |
| `review.ok` | `review-gate`, after every gate passed | **the only authorization downstream accepts.** Deleted on any gate failure and on a rejection |
| `repair-start.json` | the fixer session, first action | from here the fixer owns the worktree; partial edits are its provenance |
| `repair.json` | the fixer session, last action | the repair is complete: result digest plus the tree it produced |
| `fixer.ok` | `commit-fixer` | the repair is committed (or validated as a no-change result) |
| `decision.json` | `converge`, last action | the round's outcome, bound to HEAD, review identity, envelope generation and tree |

`converge.txt` is still a diagnostic tee and is still where you read the exit. It is never read back as state.

**Review identity** is `sha256(review_head | base | scope | contract_digest | plan_digest | allowlist_digest)`. A completed review is reused only when the envelope's `Input:` still equals the identity recomputed from the *current* contract, approved plan and allowlist. So a `feature-pin-amend` or a contract edit invalidates a finished review and it is re-run — intended. A fixer commit does not, because the recorded `review_head` is immutable.

| Verbatim signal | Meaning | Action |
|---|---|---|
| `ROUND_REUSE round=N review=<run\|reuse> attempt=<k> reason=<r>` | `round-pre`'s decision for the round, on stderr and in `node-round-pre.out`. `reason` is one of `initial`, `interrupted`, `identity`, `head-moved`, `gate-failed`, `rejected`. `reuse` means a complete envelope matched the identity and the candidate and is being gated now rather than re-discovered. This line is also round-pre's own success discriminator — its stdout is the bare JSON decision, which is not a typed line — so a `pre` that exits 0 without it reads as an untyped exit. | None. This is the line that says whether the resume cost you a review. |
| `GATE_5_input_matches=PASS\|FAIL`, and on FAIL a second line `REVIEW_INPUT=FAIL round=N expected=<id> envelope_input=[<id>] envelope_head=[<sha>]` | `review-gate` compared the envelope's `Input:` with this round's review identity and its `Head:` with the recorded candidate. Replaces v1's `GATE_4`, which compared a `Scope:` string and became ambiguous once `789b2a1` labelled the bootstrap-head comparison `delta`. Unlike `GATE_4` there is **no SKIP path**: both lanes write `review-input.json` through the same code. | `FAIL`: the envelope describes a different candidate or a different contract than this round. Resume; `round-pre` re-runs the review with `reason=identity`. |
| `REVIEW_WROTE_TREE round=N files=<n>` (review-gate or round-pre exits 1) | The worktree changed while the reviewer held it, and no `repair-start.json` claimed it. The reviewer is read-only; this is v1's unreviewed-autofix residual turned into a stop. It also fires at `round-pre` when an interrupted review attempt tries to re-snapshot a tree it edited, so an edit cannot be baked into the next snapshot. | Engineer. `git status` in the worktree, revert what the reviewer left, resume. Never widen the guard. |
| `REVIEW_UNAUTHORIZED round=N` (fix-plan prints `{"fixer":"unauthorized"}`; commit-fixer and converge exit 1) | No `review.ok` for this round, or one from a superseded envelope generation. A readable `review-summary.json` with a Ready verdict is **not** authorization — it is written before the gate's final checks precisely so that it cannot be. | Read `gate.txt` and the `GATE_` lines in `node-review-gate.out`, fix what the gate rejected, resume. |
| `FIXER_TREE_DRIFT round=N` (fix-plan or converge exits 1) | A completion record matches this review and generation, but the worktree tree does not match the tree that record produced. Something edited the candidate outside the loop. The run refuses to re-run the fixer over unaccounted edits. | Engineer. `git status` and `git diff`; revert the stray edit or accept it by hand, then resume. |
| `FIXER_INCOMPLETE round=N attempt=<k>` (commit-fixer exits 1) | The fixer left no valid `repair.json` — it failed, or ended without its closing `mark repair-done`. | Resume. `fix-plan` will say `run reason=interrupted` and the fixer resumes over its own partial tree. Recurring means the fixer prompt drifted — engineer. |
| `FIXER_ABSENT round=N` (converge exits 1) | `commit-fixer` left no `fixer.ok`, so no repair was attested on this candidate. Usually a pin-guard or commit failure in the node before. | Read `node-commit-fixer.out` for the `PIN_` line or the commit error, fix, resume. |
| `REVIEW_TREE_DRIFT round=N` (converge exits 1) | Row 9 of the closure table: the reviewer returned Ready and closure holds, but the envelope's `Head:` is not HEAD — the candidate moved after the review that approved it, with nothing applied to explain the move. v1 derived this from commit SUBJECTS in the round window; v2 compares the recorded head, which is the thing that actually matters and cannot be spoofed by a commit message. | Engineer. `git log <pre-head>..HEAD` in the worktree; revert the stray commit or accept it by hand and resume, which re-reviews at the new head. Never widen the comparison to get past it. |
| `ROUND_STATE=FAIL <detail>` (round-pre, exits 1) | `round-state.py` refused to open or advance the round. The common cause is a missing or empty `files-allowlist.json`: an absent allowlist used to read as "allow nothing", which stages nothing and empties the candidate, so it is a stop instead. | Read the detail. A missing allowlist means the plan gate never wrote one — fix the artifacts dir and resume. |
...
| `PIN_OK pins=<n> changed=<n>` / `PIN_CHANGED symbol=<s> allowed=<text>` / `PIN_BREACH symbol=<s> file=<f>` / `PIN_UNRESOLVED symbol=<s> file=<f> reason=<r>`, wrapped by `PIN_GUARD=FAIL round=N` when `commit-fixer` is the caller | The pin guard, run on the **staged index** in `commit-impl` and `commit-fixer` — what will actually be committed, not what happens to be in the worktree. `PIN_CHANGED` is informational (the plan allowed that change). `PIN_BREACH` and `PIN_UNRESOLVED` fail the node with the edit left staged. A symbol the guard cannot find is a stop, never a pass: a silently missed span is the worse failure. | `PIN_BREACH`: revert the staged hunk and resume, or `feature-pin-amend` (below), or reject and re-plan. `PIN_UNRESOLVED`: read `round-N/pin-spans.json`; the extractor is a line-anchored heuristic and tree-sitter is the documented upgrade. |
...
| `PREFLIGHT=FAIL claude session expires in <m> min` | The Claude login's **refresh** token lapses inside 3 hours, and the run is budgeted for ~2 unattended. Read from the macOS keychain item `Claude Code-credentials`, or `~/.claude/.credentials.json` on Linux. An unreadable store is `PREFLIGHT_WARN`, never a failure. Measured 2026-09-16: `expiresAt` is the ACCESS token, renewed silently on a ~3 h cycle, so it is not the field to gate on. | Run `/login` in Claude Code, then start the run. |
| `PREFLIGHT=FAIL <VAR>=<addr> is not in OTP_TEST_WHITELIST_EMAILS` | `joint-api-mcp-e2e.sh` mints a JWT for both `ARCHON_OTP_TEST_EMAIL` and `ARCHON_OTP_SECOND_EMAIL`, and the api refuses an address the stage `.env` does not whitelist. Caught at preflight instead of an hour later at integration. | Add the address to `OTP_TEST_WHITELIST_EMAILS` in `api/.env` and restart the api, or export the variable to an address already on the list. |

**The closure table.** `converge` evaluates these in order, first match wins, and writes the result to `decision.json`:

| # | Condition | Result |
|---|---|---|
| 1 | any ledger entry `pin_conflict` | blocked — `PIN_CONFLICT` |
| 1b | any `filed` entry (the fixer's `cross_repo` partition) | blocked — `CROSS_REPO_FINDING` |
| 2 | `fixer-result.incomplete > 0` | progressed, next round `full` |
| 3 | `Not ready` with no P0/P1 open, applied or regressed | blocked — `NOT_READY_WITHOUT_BLOCKER` |
| 4 | a `verify` round found a new P0/P1 | progressed, next round `full` |
| 5 | this round applied a P0, or any applied entry set `design_expanded` | progressed, next round `full` |
| 6 | anything applied | progressed, next round `verify` |
| 7 | `Not ready`, nothing applied | progressed, next round `full` — the cap bounds this |
| 8 | Ready, closure holds, envelope `Head:` == HEAD | **converged** + `residuals.json` + the promise |
| 9 | Ready, closure holds, `Head:` ≠ HEAD | blocked — `REVIEW_TREE_DRIFT` |
| 10 | Ready, closure does not hold | progressed, next round `verify` |

The pre-round cap in `round-pre` is what bounds rows 2 and 4–7 and 10; at the cap it is `ROUND_CAP_REACHED` unless a human wrote `accept-residuals.txt`. Writing that file is still a human act and agents still never write it.

**`yield-stop.txt` no longer converges the parent lane.** It was the opt-in escape hatch for a loop that had stopped yielding — two rounds that applied nothing while the reviewer re-raised what the ledger had already waived. Closure convergence replaces it: a round that applies nothing and closes nothing lands on table row 7 or row 10 and is bounded by the cap, and `REVIEW_RERAISE` is printed as a report rather than as a qualifier. Writing the file on `full-sdlc-api` now does nothing except fail `chain-acceptance.py` clause 1, which still greps for it. The lite lane's converge is its own body and keeps its own single-round convergence, `LITE_FIXES_UNREVIEWED` included.

**The risk-delta pilot must re-capture `full-sdlc-api-codex` after this lands.** `setup/review_delta_workflow.py` pins the review-loop to v1's seven-node list and raises `DeltaWorkflowError` on anything else, so a v2 capture is refused — typed, not rewritten wrongly. `bugfix-codex` still carries the v1 shape and is unaffected.
...
| `PLAN_CONVERGED round=N (cap reached: K applied blocking findings not re-critiqued; routed to the human gate, see plan-cap-unverified.json)` | The cap round's critique carried no P0 at ≥75, the reviser applied every P1 at ≥75 and declined none, and `plan-shape.sh` passed. Only a critic's re-read of the final revision is missing, so the run proceeds to the plan gate instead of dying. `plan-cap-unverified.json` lists the applied-but-unchecked findings; `plan-render-gate` fails unless the packet's CRITIC section flags them (`data-cap-unverified`). | At the gate, read those `what_changed` edits against the plan yourself: nobody else reviewed them. Approve or reject as for any plan. |
| `PLAN_ROUND_CAP round=N cap=N` | Round cap hit (default 3, override via `plan-round-cap.txt`; repository-scope preflight writes 3, raised from 2 after chain 42b42a13 died at 2 with its last blocking finding applied) while a disagreement remains: a P0 at ≥75, a declined P0/P1 at ≥75, or a blocking finding the reviser neither applied nor declined. A cap round with every blocking finding applied routes to the gate instead (row above). **Fires in two places**: `plan-round-pre` (before a round is spent — the primary enforcement, since a resumed `loop_group` gets a fresh iteration counter) and again in `plan-converge` (belt, for the case `max_iterations` alone would otherwise hit first). | Raise the cap (`echo N > <artifacts>/plan-round-cap.txt`) or accept the loop's last state by hand, then resume. | **Accept by hand:** write `<artifacts>/plan-accept.txt` (one line: who and why) and resume — `plan-converge` converges when the last critique carries no P0 at ≥75 confidence and `plan-shape.sh` still passes; a P0 still stops. Mirror of `rca-plan-accept.txt` (§12).
...
Same caveat as §3a: read from `workflows/full-sdlc-api.yaml` and `workflows/bugfix.yaml`, not from an observed run. The api lane runs this between `gate-tests` and `commit-impl`/`reader-audit`; the bugfix lane runs it between the fix loop and `deslop-commit` (its bugfix-only rows are marked below; the rest apply to both). The deslop **writer** (`deslop`) cleans the uncommitted implementation against five guards (complexity, tautological tests, YAGNI, open/closed, narrating comments); `deslop-verify` is a `loop_group` (`deslop-fix-pre` → `deslop-fix` → `deslop-recheck` → `deslop-review` → `deslop-review-gate`, capped at 2 iterations) that mechanically re-checks the writer's work and then has an independent reviewer session judge it. `deslop-fix` runs only on the iteration after a `DESLOP=DIRTY` verdict (it is skipped by `when:` otherwise) and applies exactly the blocking findings. The gate must exit 0 for that to work: archon 0.10.1 fails a whole `loop_group` on any failed body node, so a gate that exited 1 on DIRTY ended the loop at iteration 1 (run 180f2453, before this change). Every path into `review-loop` depends on `deslop-verify` — no un-deslopped code reaches a reviewer.

| Verbatim signal | Meaning | Action |
|---|---|---|
| `DESLOP_GATE=FAIL <guard> round=N` — `<guard>` one of `typecheck`, `lint`, `unit`, `scope`, `slop` (both lanes). Bugfix also has `DESLOP_GATE=FAIL repro no longer green round=N rc=<RP>` — note `round=N` comes BEFORE `rc=<RP>` there, breaking the `<guard> round=N` template's suffix order (see the harness-error variant below for its own separate `rc=97`) | `deslop-recheck`'s own mechanical re-verification failed — the writer's cleanup or the automatic fixer's edits broke something, or (bugfix) moved the repro off green. After a fix, the recheck consumes `deslop-fix-pending.txt` BEFORE these checks run, so a check the fixer broke does not buy another fixer pass. | Hand-fix the worktree (the failure prints the relevant log tail), then resume — resume re-enters the `loop_group` with a fresh iteration, so `deslop-recheck` re-runs and re-takes the checkpoint. The fixer does not run again: the marker is already consumed. |
...
| `DESLOP=DIRTY round=N blocking=N` followed by `DESLOP_RETRY=PASS round=N dirty_rounds=1` | The reviewer's `deslop-review.json` has ≥1 finding at confidence 75 or 100 — the gate-derived verdict (never the reviewer's own `verdict` field alone) says DIRTY. Below the cap this is a retry, not a stop: the gate exits 0 without the promise, writes the findings it counted as blocking to `deslop-round-N/blocking-findings.json`, and leaves `deslop-fix-pending.txt`. The next iteration's `deslop-fix` applies only those findings (same scope and rules as the writer; its edits are declared only in `deslop-fix-result.json`, which the recheck copies to `deslop-round-N/fix-result.json` for the reviewer), then `deslop-recheck` validates that result, consumes the marker and re-checks, and a fresh reviewer judges the tree. | None — the run continues on its own. A hand fix is needed only when the second verdict is also DIRTY (`DESLOP_ROUND_CAP`, next row). |
| `DESLOP_ROUND_CAP round=N dirty_rounds=N` | The durable DIRTY counter (`deslop-dirty.txt`, counts verdicts reached, not mechanical-recheck failures) hit 2: the automatic fixer already had its one pass and the reviewer still blocks. No `deslop-fix-pending.txt` is written at the cap. | **Hand fix** the findings in `deslop-round-N/blocking-findings.json`, then resume — resume re-enters the loop fresh with no fixer pass, re-checkpoints, and gets a fresh reviewer; another DIRTY verdict stops here again. There's no auto-raise recipe (unlike `plan-round-cap.txt` / `round-cap.txt`) — treat as a stop. |
| `DESLOP_FIX=FAIL deslop-fix-pending.txt is not an integer: [<v>]` / `DESLOP_FIX=FAIL no blocking-findings.json for round=N` (`deslop-fix-pre`; the integer check repeats in `deslop-recheck`) / `DESLOP_GATE=FAIL no deslop-fix-result.json round=N` / `DESLOP_GATE=FAIL deslop-fix-result.json unparseable round=N` / `DESLOP_GATE=FAIL deslop-fix-result.json does not answer each blocking finding exactly once round=N` (`deslop-recheck`) | The pending-fix marker is junk or points at a round with no findings snapshot, or the fixer ended without a usable result for that round: a wrong or non-integer `round` counts as unparseable, and `fixed` + `not_fixed` must hold each finding of `deslop-round-<marker>/blocking-findings.json` exactly once, matched on guard, file and line. The marker is consumed only after a valid result, so it survives these stops. | A missing/unparseable fixer result: plain resume — the loop re-enters, `deslop-fix-pre` clears the stale result and the fixer runs again. A junk marker or missing snapshot means a hand-edited artifacts dir: fix or delete `deslop-fix-pending.txt` (deleting it skips the automatic fix), then resume. |
| `DESLOP_GATE=FAIL deslop-result.json changed after the writer round=N` | `deslop-result.json` is the writer's declaration, and the bugfix reviewer excuses out-of-guard edits only against it. The first recheck that reaches it before any fixer ran records its sha256 in `deslop-result.sha256`; every later round must match. A fixer that appended its own `passes` (or deleted the record) trips this. | Engineer stop: restore `deslop-result.json` to the recorded bytes (the fixer's declaration belongs in `deslop-fix-result.json`), then resume. A hand fix at `DESLOP_ROUND_CAP` never needs to edit it. |
| `DESLOP_RECHECK=FAIL deslop-round.txt rewound: round=N was already checked` (`deslop-recheck`) / `DESLOP_REVIEW=FAIL deslop-dirty.txt rewound: file=<d> recorded=<r> round=N` (`deslop-review-gate`) | A counter went backwards. The fixer runs between two gates and is told not to touch gate state; every recheck writes `recheck.json` into the round it opens, and every DIRTY verdict records its count in `deslop-round-N/dirty-count.txt`, so a lower `deslop-round.txt` or `deslop-dirty.txt` is detected instead of reusing a round or buying a fix past the cap. | Engineer stop: set the counter back to the highest recorded value (the last `deslop-round-*` with `recheck.json`, or the highest `dirty-count.txt`), then resume. |
| `DESLOP_REVIEW=FAIL coverage incomplete round=N <reason>` / `DESLOP_REVIEW=FAIL malformed finding round=N <reason>` / `DESLOP_REVIEW=FAIL verdict inconsistent round=N declared DIRTY with 0 blocking findings` | Fail-closed schema validation on `deslop-review.json`: a missing/wrong-status coverage key, empty evidence, an out-of-enum guard/confidence/line, a finding `file` holding a line break, control character or angle bracket (it is printed on the exit-0 retry path, where it could forge `<promise>DESLOP_CLEAN</promise>`), or a declared verdict that disagrees with the gate's own re-derivation from `findings[]`. `deslop-recheck` deletes the previous round's `deslop-review.json` after its checkpoint, so a reviewer that writes nothing fails as `no deslop-review.json` rather than being judged by the old file. | Resume re-runs `deslop-review` fresh (loop_group re-enters). **Recurring on the same shape of failure means the reviewer prompt drifted — engineer**, not a resume candidate. |
...
- Completed AI nodes **outside a failed `loop_group`** never re-run (proven across 6+ resumes on one run); only failed bash gates re-execute. Resuming is cheap. The exception is the next bullets: a failed loop re-enters with a fresh iteration and re-runs its body AI nodes.
...
**Engine facts the v2 review loop is built on, probed 2026-09-16 on archon CLI 0.10.1** (a throwaway `when-probe.yaml`, three runs). These are load-bearing, not trivia — each one is the reason a node in `full-sdlc-api`'s `review-loop` carries the key it carries, and `setup/tests/test_node_review_loop_shape.py` pins the YAML to them.

1. **`when:` on a node inside a `loop_group` is honoured before the node is invoked**, and it can read a JSON field of a sibling node's output. The sibling's stdout must be a **bare JSON object** — which is why `round-pre` and `fix-plan` send every human line and their tee to stderr and print exactly one JSON line. This is also the sharpest edge in the whole lane: an unparseable `when:` SKIPS its node and the run still reports SUCCESS (see the "three green runs that did nothing" note in §3), so a stray `echo` on those two nodes' stdout does not produce a wrong decision, it produces a review that never happened.
2. **A dependent of a `when:`-skipped node is itself skipped.** So `review-gate` after a skipped `review`, `commit-fixer` after a skipped `fixer`, and `converge` would all vanish on a reuse round.
3. **`trigger_rule: all_done` on the dependent makes it run anyway**, after a skipped *or failed* upstream (the same key `babysit.yaml:76` uses). That is why `review-gate`, `commit-fixer` and `converge` carry it — and note the *failed* half: v1's converge was protected from a failed `review-gate` only by the skip chain, so v2's converge has to check `review.ok` itself. It does.
4. **A `loop_group`'s completion promise is honoured only from the group's LAST node.** Emitted from the first node, the group ignored it and ran to `max_iterations`. So `converge` is the only node that may print `<promise>REVIEW_CONVERGED</promise>`, and the terminal replay of an already-converged round lives there too.
5. **Per-node `retry:` is not honoured** (v1 probe, unchanged): a retry probe produced one `node_started` and one `node_failed`, never a second attempt. A transient provider stream drop is cured by a resume, not by a retry key.
...
- **Deslop gate budget** (`deslop`/`deslop-verify`, §3b): the writer is $4 (sonnet), the reviewer is $2 (sonnet) — a clean pass is **+$6**. A first `DESLOP=DIRTY` verdict now buys one automatic iteration in the same process: the fixer ($4, sonnet) plus a second reviewer ($2) — **+$12** worst case before `DESLOP_ROUND_CAP`. A resume after the cap re-runs only `deslop-recheck` + `deslop-review` (no writer, no fixer), another $2. Same caveat — design-derived, not yet measured live.
...
| `RCA_PLAN_ROUND_CAP round=N cap=N` | Round cap (default 3, override `rca-round-cap.txt`) hit — fires in both `rca-round-pre` and `rca-converge`, same belt-and-braces reason as §3a. | Raise the cap (`echo 4 > <artifacts>/rca-round-cap.txt`) and resume for another round; or **accept by hand**: write `<artifacts>/rca-plan-accept.txt` (one line: who and why) and resume — the next `rca-converge` converges on the current plan when the last critique carries no P0 at ≥75 confidence (the P1/P2 residue is yours), and still stops with `... (rca-plan-accept.txt present but critique has P0=N ...)` when a P0 remains. Mirror of `accept-residuals.txt` (§3). |
...
| `RCA_INVESTIGATION_REQUIRED reason=reproduction-or-causal-proof-missing ticket=open no_implementation=true` | The report is too thin to carry an implementation: no reproduction and no causal proof selects a fix site. Also a valid open investigation, not a malformed RCA. | Gather the missing reproduction/occurrence evidence. Never fabricate a `separate-ticket` authority to get past the contract; an unprovable symptom is `unresolved`. |
| Terminal `status=failed` with a `discriminator` field | `supervise_exact_run` read the run's `gate-status.txt`. `rca-shape.sh` persists exactly one typed STOP there, stamped with the caller's token (`RCA_GATE=` or `RCA_PLAN_SHAPE=`), because a failing node's stdout does not survive the executor's failure path -- it reports a fragment of the node's own script source instead. Successes are never persisted and the file is truncated per call, so an empty file means this check passed and the run died elsewhere. | Route on the discriminator's token: `RCA_GATE=` uses the rca-gate rows above, `RCA_PLAN_SHAPE=` the planning-loop rows. `RCA_GATE=FAIL contract crashed without a typed line` means bugfix-contract.py raised -- read the run log for the traceback. |
...
| `beyond_five_guards` finding in `deslop-review.json` (VERSION 2026.08.28-2, design-only) | Bugfix-only sixth reviewer value (findings-only, not a coverage key). This lane is causal-minimal: the deslop writer may fix only the five guards (complexity, tautological tests, YAGNI, open/closed, comments) and must report anything else under `reported_not_fixed`. The reviewer diffs the writer's actual edits against what it declared and files `beyond_five_guards` for any edit that is neither one of the five guards nor listed in `deslop-result.json`'s `passes[]` — it blocks at confidence 75/100 exactly like the other five. `deslop-result.json` is frozen after the writer, and a fixer's `fixed` entry never excuses an out-of-guard edit: a claimed revert that is still in the tree is filed again. | Same handling as any `DESLOP=DIRTY` (§3b): the first time, `deslop-fix` restores the flagged region to its HEAD content automatically — HEAD is the committed fix, and red-sha content would remove the fix. `deslop-recheck` then diffs each flagged hunk of the reviewed checkpoint against HEAD and stops with `DESLOP_GATE=FAIL beyond_five_guards edit not reverted to HEAD file=<f>:<line> round=N` if it is still there (a line outside every reviewed hunk requires the whole file back at HEAD), or `DESLOP_GATE=FAIL fixer moved HEAD round=N expected=[<sha>]` if the fixer committed; only a second blocking verdict (`DESLOP_ROUND_CAP`) needs a hand fix, then resume. A cleanup the writer correctly listed under `reported_not_fixed` and did not make is never a finding. |
...
## 14. Lite lanes (`full-sdlc-api-lite`, `bugfix-lite`) and the routing envelope
...
|---|---|---|---|
| `lite-envelope-pre` | bugfix | `repro` (report's `## Repro` block gave one allow-listed test-runner command with no shell metacharacters, plus observed output), `repo_hint` ≠ unknown, `triage` | `evidence-plan.json`, `triage.json` |
| `lite-envelope` | api | `premises` (spec has no `## Premises to verify`), `triage`, `files` ≤ `max_files` non-test, `test_files`, `hot_paths` (anchored prefix on `<repo>/<path>`), `reader_audit` (columns must be `[]`), `impact` (UNAVAILABLE fails), `d1_callers` | `files-allowlist.json`, `reader-audit.json`, `impact.json`, `triage.json` |
| `lite-envelope` | bugfix | `triage`, `repo` (never `both`), `files` (fix-plan ∪ allowlist), `test_files`, `hot_paths`, `chain_links`, `impact`, `d1_callers` | `fix-plan.json`, `files-allowlist.json`, `causal-chain.json`, `repo.json`, `impact.json`, `triage.json` |
| `lite-envelope-post` | both | the same checks re-run AFTER approval on the artifacts as they stand, with a fresh `impact.json` and a fresh `triage-post.json` (the pre-gate `triage.json` is not consulted) | same |

Every stage writes `envelope-<stage>.txt` on both outcomes (node stdout does not survive a resume);
the gate packet renders `envelope-plan.txt`, the PR body renders `envelope-post.txt`.

A third evaluation, `lite-envelope-final`, runs **after `exit-gate`** on both lanes: a `SCOPE_BREACH` widening of `files-allowlist.json` is a sanctioned human act at `converge` and again at `exit-gate` (both run `check-scope.py`), so the envelope is re-read on the allowlist as it finally stands and `envelope-post.txt` is overwritten before `prbody` renders it. A widening past a cap stops the run there; it never ships.

**`ROUTE=FULL reason=<check>[,<check>...]`** is a hard stop; every check is evaluated and each failing one prints its own `ENVELOPE <check>=FAIL <detail>` line above it, so a hot-path hit is never hidden behind a triage refusal. The action is always the same: relaunch the
**same spec** on the full lane (`full-sdlc-api` / `bugfix`). Never edit `lite-envelope.json`,
`triage.json`, `files-allowlist.json` or `fix-plan.json` to pass; never resume past it. Reasons:
`repro`, `repo_hint`, `triage`, `premises`, `files`, `test_files`, `hot_paths`, `reader_audit`,
`repo`, `chain_links`, `impact`, `d1_callers`, `malformed` (a missing or unparsable input; fail-closed,
not a pass).

**Triage** (`triage.json`, written by `ralplan` / `intake`; `triage-post.json` by `lite-impact-post`):
`S` passes, `L` never passes, `M` passes only when the spec carries the literal line `Lane: lite-ok`.
Writing that line is a human act, the same convention as `files-allowlist.json` (§3).
...
nodes, `<loop>.<id>.*` for loop bodies). A retained node is the parent's bytes. `package.sh`
regenerates and diffs before the secret gate; `LITE_DRIFT=FAIL <lane>` means a parent edit was not
regenerated into its twin or someone edited the YAML: fix the source, regenerate, repackage.
Tests: `setup/tests/test_lite_envelope.py`, `test_derive_lite.py`, `test_lite_converge.py`,
`test_lite_red_gate.py`.

### 14d. Calibration (fill from real runs)

The envelope numbers are policy, not measurement. Paste each lite run's `envelope-post.txt` and
cost here for the first five runs, then revisit `max_files` and `hot_paths`.

| date | lane | ticket | files | d1 | route | cost-eq | outcome |
|---|---|---|---|---|---|---|---|
| 2026-08-29 | bugfix-lite | ENG-3816 | n/a | n/a | FULL reason=repro (pre) | ≈$2 (intake only) | relaunched on bugfix as designed |
| 2026-08-29 | bugfix-lite | ENG-3808 | 1/4 | 1/10 | FULL reason=chain_links 7/4 (plan) → cap raised to 7, resumed | ≈$14 to plan stage | chain length is explanation depth, not fix size; 4 refused an S ticket |
| 2026-08-29 | full-sdlc-api-lite | ENG-3692 | see allowlist | GATHERED | FULL reason=triage M (additive DTO field = contract change) | ≈$8 | relaunched with `Lane: lite-ok` (human override) |
| 2026-08-29 | full-sdlc-api-lite | ENG-3657 | 3/4 | 2/10 | FULL reason=triage,hot_paths (oauth/ ×3) | ≈$8 | probe: refused as designed; not relaunched |
| 2026-08-29 | full-sdlc-api-lite | ENG-3809 | ok | 15/10 | FULL reason=triage(L),d1_callers | ≈$8 | probe: refused as designed (lambda ingest handler, race) |
...
Repository-list Codex controls also enforce native-child model parity through a
trusted Codex hook. The dedicated Codex home must be private before guarded
install (`0700` home; config/manifest not group/world writable). If a native
child request is denied, relaunch or revise the calling prompt/tool request with
the printed `model=<chain-model>` and `reasoning_effort=<chain-effort>` values;
do not bypass hook trust or hand-edit the manifest.
