...
|---|---|---|
| `REVIEW_MODE=full base=<sha>` / `REVIEW_MODE=delta base=<sha>` | `round-pre` decided this round's review scope and wrote it to `round-N/review-mode.txt` + `round-N/review-base.txt`. `full` reviews against `origin/main`; `delta` reviews only what landed since `<sha>`, because everything before it was already reviewed and its findings recorded. Informational — the node still types its exit with `ROUND=N head=<sha>`. | None. To force a full round, delete `round-N/review-mode.txt` before resuming. |
