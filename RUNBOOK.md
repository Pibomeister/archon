...
| `DESLOP_FIX=FAIL deslop-fix-pending.txt is not an integer: [<v>]` / `DESLOP_FIX=FAIL no blocking-findings.json for round=N` (`deslop-fix-pre`) / `DESLOP_GATE=FAIL no deslop-fix-result.json round=N` / `DESLOP_GATE=FAIL deslop-fix-result.json unparseable round=N` (`deslop-recheck`) | The pending-fix marker is junk or points at a round with no findings snapshot, or the fixer ended without a result file for that round (a wrong `round` counts as unparseable). The marker is consumed only after a valid result, so it survives these stops. | A missing/unparseable fixer result: plain resume — the loop re-enters, `deslop-fix-pre` clears the stale result and the fixer runs again. A junk marker or missing snapshot means a hand-edited artifacts dir: fix or delete `deslop-fix-pending.txt` (deleting it skips the automatic fix), then resume. |
