...
| `GATE_4_scope_matches_mode=PASS\|FAIL\|SKIP` *(v1 lanes only; `full-sdlc-api` replaced it with `GATE_5_input_matches`, §3c)* | `review-gate` compared `round-N/review-mode.txt` with the `Scope:` line the reviewer opened its envelope with. `SKIP` means no mode file: the normal state for the lite lanes, which overlay `round-pre` and never write one, and for any install predating `review-mode.py`. | `PASS`/`SKIP`: none. `FAIL`: the row below. |
| `REVIEW_SCOPE=FAIL mode=<m> envelope=[<Scope line>]` (review-gate exits 1) *(v1 lanes only)* | The reviewer reviewed a different scope than the round asked for — a delta round that re-read everything, or a full round that read only a slice. The round is billed and its verdict describes the wrong diff, so the gate refuses it. | Resume: the next round re-reviews with a fresh reviewer session. Recurring on the same round means the review prompt drifted — engineer. |
| `REVIEW_AUTOFIX_ONLY round=N files=<n>` *(v1 lanes only; `full-sdlc-api`'s reviewer is read-only and `commit-fixes` is deleted, so there is nothing left to commit on its behalf — the equivalent stop there is `REVIEW_WROTE_TREE`, §3c)* | Every commit in this round's window was `fix(review): apply autofix feedback` — edits the REVIEW session left in the worktree, which no reviewer has read. `converge` therefore treats HEAD as unmoved, so a reviewer-session edit can no longer buy the loop another round. The files are listed in `review-autofix-unreviewed.txt`. | None automatically. Read `review-autofix-unreviewed.txt` before merging: those hunks are reviewer-unverified, the same class as `lite-fixes-unreviewed.txt`. |
| `REVIEW_TREE_DRIFT round=N sha=<sha>` (converge exits 1) | On `full-sdlc-api` this is row 9 of the closure table (§3c): a Ready verdict whose envelope `Head:` is not HEAD. On the v1 lanes: a commit in this round's window carries neither `apply fixer feedback` nor `apply autofix feedback` — a hand edit, a stray commit, or a resume that landed work from outside the loop. The unexpected subjects are printed beneath it. | Engineer. Run `git log <pre-head>..HEAD` in the worktree, decide whether the commit belongs, then revert it or accept it by hand and resume. Never widen the subject filter to get past this. |
