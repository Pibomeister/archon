...
Read `$ARCHON_LAYER/setup/review-contract.md` now, sections 1, 4, 6, 7 and 8 in
...
YOUR FIRST ACTION, before reading anything else, is exactly:
    ...
    python3 $ARCHON_LAYER/setup/round-state.py mark "$ARTIFACTS_DIR" review-start

YOUR LAST ACTION, after the envelope file is written, is exactly:

    python3 $ARCHON_LAYER/setup/round-state.py mark "$ARTIFACTS_DIR" review-done "$ARTIFACTS_DIR/round-$N/review-envelope.txt"
