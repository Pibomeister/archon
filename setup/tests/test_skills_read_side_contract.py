#!/usr/bin/env python3
"""Read-side contract of the skills layer across the five lanes and their
codex and grok twins: the stage-skills node exists with the right edges, only
implement, fix and fixer are told about skills.md, the two prompt lines are
byte-identical everywhere, and the doctrine lock carries them."""
import json
import re
import unittest
...
GROK = tuple(f"{lane}-grok" for lane in LANES)
ALL = LANES + TWINS + GROK
PROMPT_LINES = (
...
class StageSkillsNode(unittest.TestCase):
    ...
    def test_when_only_on_the_api_parent_and_its_twin(self):
        for lane in ALL:
            with self.subTest(lane=lane):
                n = nodes(lane)["stage-skills"]
                if lane in ("full-sdlc-api", "full-sdlc-api-codex", "full-sdlc-api-grok"):
                    self.assertEqual(n["when"], "$feature-phase.implement == 'yes'")
                else:
                    self.assertNotIn("when", n)

    def test_depends_on_edges(self):
        for lane in ("full-sdlc-api", "full-sdlc-api-codex", "full-sdlc-api-grok"):
            ns = nodes(lane)
        ...
        for lane in ("full-sdlc-api-lite", "full-sdlc-api-lite-codex", "full-sdlc-api-lite-grok"):
            ns = nodes(lane)
            self.assertEqual(ns["stage-skills"]["depends_on"], ["bootstrap"])
            self.assertEqual(ns["implement"]["depends_on"], ["lite-envelope-post", "stage-skills"])
        for lane in ("full-sdlc-web", "full-sdlc-web-codex", "full-sdlc-web-grok"):
            ns = nodes(lane)
            self.assertEqual(ns["stage-skills"]["depends_on"], ["bootstrap"])
