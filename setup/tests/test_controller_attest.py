...
class AttestControllerTest(unittest.TestCase):
    ...
    def test_proof_seal_is_private_external_and_verified(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            result = self.run_cli(CONTRACT, "write-proof-manifest", "--artifacts", ad)
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            out = root / "private" / "proof.json"
            created = self.run_cli(SCRIPT, "proof", "--artifacts", ad, "--chain-state", state, "--out", out)
            self.assertEqual(created.returncode, 0, created.stdout + created.stderr)
            self.assertEqual(out.stat().st_mode & 0o777, 0o600)
            self.assertIn("authority_mac", json.loads(out.read_text()))
            verified = self.run_cli(SCRIPT, "proof", "--verify", "--artifacts", ad, "--chain-state", state, "--out", out)
    ...
    def test_approval_seal_binds_final_critic_manifest_role_and_mac(self):
        for provider in ("claude", "codex"):
            with self.subTest(provider=provider), tempfile.TemporaryDirectory() as td:
                ...
                self.assertEqual(self.run_cli(CONTRACT, "write-proof-manifest", "--artifacts", ad).returncode, 0)
                self.assertEqual(self.run_cli(CONTRACT, "write-approval-manifest", "--artifacts", ad).returncode, 0)
                ...
                created = self.run_cli(SCRIPT, "approval", "--artifacts", ad, "--chain-state", state, "--out", out)
                self.assertEqual(created.returncode, 0, created.stdout + created.stderr)
                seal = json.loads(out.read_text())
                self.assertEqual(seal["role"], "final-critic")
                self.assertEqual(seal["provider"], provider)
                self.assertEqual(self.run_cli(SCRIPT, "approval", "--verify", "--artifacts", ad, "--chain-state", state, "--out", out).returncode, 0)
                seal["run_id"] = "b" * 32
                out.write_text(json.dumps(seal))
                out.chmod(0o600)
                tampered = self.run_cli(SCRIPT, "approval", "--verify", "--artifacts", ad, "--chain-state", state, "--out", out)
    ...
    def test_rejected_final_critic_cannot_issue_approval_seal(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            result = self.run_cli(SCRIPT, "approval", "--artifacts", ad, "--chain-state", state, "--out", root / "approval.json")
    ...
    def test_lite_approval_seal_is_current_and_tamper_evident(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(self.run_cli(CONTRACT, "write-proof-manifest", "--artifacts", ad).returncode, 0)
            self.assertEqual(self.run_cli(CONTRACT, "write-approval-manifest", "--artifacts", ad).returncode, 0)
            self.assertEqual(self.run_cli(CONTRACT, "write-lite-approval-manifest", "--artifacts", ad).returncode, 0)
            out = root / "private" / "lite.json"
            created = self.run_cli(SCRIPT, "lite-approval", "--artifacts", ad, "--chain-state", state, "--out", out)
            self.assertEqual(created.returncode, 0, created.stdout + created.stderr)
            self.assertEqual(self.run_cli(SCRIPT, "lite-approval", "--verify", "--artifacts", ad, "--chain-state", state, "--out", out).returncode, 0)
            plan = json.loads((ad / "fix-plan.json").read_text())
            plan["approach"] = "stale gate edit"
            (ad / "fix-plan.json").write_text(json.dumps(plan))
            current = self.run_cli(CONTRACT, "validate-current-manifest", "--artifacts", ad, "--kind", "lite-approval")
