...
class TwinShape(unittest.TestCase):
    def check(self, target):
        ...
        if target in dc.INJECTED_GUARD_TARGETS:
            self.assertEqual(tn[0]["id"], "codex-control-guard")
            self.assertEqual(tn[1].get("depends_on"), ["codex-control-guard"])
            tn_compare = tn[1:]
        else:
            tn_compare = tn
        self.assertEqual([n["id"] for n in pn], [n["id"] for n in tn_compare], "node id order drifted")
        for p, t in zip(pn, tn_compare):
            nid = p["id"]
            # model rules
            ...
            for k in set(p) | set(t):
                ...
                if k == "approval" and target in dc.GUARDED_TARGETS:
                    ...
                    continue
                if k == "depends_on" and target in dc.INJECTED_GUARD_TARGETS and nid == "preflight":
                    self.assertEqual(t.get(k), ["codex-control-guard"])
                    continue
...
class TransformStats(unittest.TestCase):
    def test_guard_and_mirror_counts(self):
        for t in MAIN_TARGETS:
            ...
            expected_rewrites = 12 if t == "bugfix" else (6 if t in (dc.LITE_TARGETS | {"full-sdlc-api"}) else 0)
