...
if [ "$N" -gt "$CAP" ]; then echo "ROUND_CAP_EXCEEDED round=$N cap=$CAP (acceptance covers the cap round only; raise round-cap.txt to spend more)" >&2; exit 1; fi
...
python3 $ARCHON_LAYER/setup/round-state.py pre "$ARTIFACTS_DIR"
