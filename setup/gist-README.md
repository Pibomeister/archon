...
This gist ships nineteen workflows: the nine Claude workflows (`babysit`, `bugfix`, `bugfix-lite`, `bugfix-smoke-deployed`, `cleanup`, `full-sdlc-api`, `full-sdlc-api-lite`, `full-sdlc-web`, `register-probe`) plus five generated Codex twins and five generated Grok twins. The two `-lite` lanes, all `-codex` lanes, and all `-grok` lanes are generated; never hand-edit them. If a workflow is missing, compare against `<root>/.archon/VERSION`.
